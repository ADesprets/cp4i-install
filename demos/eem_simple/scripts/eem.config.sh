...
function eem_run_all () {
  ...
  decho $lf_tracelevel "oc -n $VAR_APIC_NAMESPACE get APIConnectCluster -o=jsonpath='{.items[?(@.kind=="APIConnectCluster")].status.endpoints[?(@.name=="jwksUrl")].uri}'"
  ...
  if ! oc -n ${VAR_EEM_NAMESPACE} get secret $VAR_EEM_APIC_INGRESS_CA_CERT_SECRET_NAME >/dev/null 2>&1; then
    ...
    decho $lf_tracelevel "oc -n $VAR_EEM_NAMESPACE patch EventEndpointManagement "${VAR_EEM_INSTANCE_NAME}" --type merge -p '{\"spec\": {"manager": {\"apic\": {\"jwks\": { \"endpoint\": \"$lf_jwks_url\"}}}}}'"
    ...
    decho $lf_tracelevel "oc -n $VAR_EEM_NAMESPACE patch EventEndpointManagement "${VAR_EEM_INSTANCE_NAME}" --type merge -p \"{\"spec\":  {\"manager\": {\"tls\": {\"trustedCertificates\": [{\"certificate\": \"ca.crt\"},{\"secretName\": \"$VAR_EEM_APIC_INGRESS_CA_CERT_SECRET_NAME\"}]}}}}\""
