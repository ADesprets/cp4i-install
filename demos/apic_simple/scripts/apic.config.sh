...
function create_apic_resources() {
  ...
  if [ $(echo $registryURLfakeAPI | jq .status ) = "404" ] || [ -z "$registryURLfakeAPI" ] || [ "$registryURLfakeAPI" = "null" ]; then
    ...
    adapt_file ${MY_APIC_SIMPLE_DEMODIR}config/ ${MY_APIC_GEN_CUSTOMDIR}config/ AuthenticationURL_Registry_res.json

    decho $lf_tracelevel "curl -sk \"${PLATFORM_API_URL}api/orgs/admin/user-registries\" -H 'accept: application/json' -H \"authorization: Bearer cm_token\" -H 'content-type: application/json' -H \"Connection: keep-alive\" --compressed --data-binary \"@${MY_APIC_GEN_CUSTOMDIR}config/AuthenticationURL_Registry_res.json\""
  ...
  if [ 2 -gt 3 ]; then
    ...
    adapt_file ${MY_APIC_SIMPLE_DEMODIR}config/ ${MY_APIC_GEN_CUSTOMDIR}config/ ConfiguredUserRegistry_res.json
  ...
  if [ $(echo $oauthProviderURL | jq .status ) = "404" ] || [ -z "$oauthProviderURL" ] || [ "$oauthProviderURL" = "null" ]; then
    ...
    adapt_file ${MY_APIC_SIMPLE_DEMODIR}config/ ${MY_APIC_GEN_CUSTOMDIR}config/ NativeOAuthProvider_res.json
  ...
  if [ 2 -gt 3 ]; then
    ...
    adapt_file ${MY_APIC_SIMPLE_DEMODIR}config/ ${MY_APIC_GEN_CUSTOMDIR}config/ ConfiguredOAuthProvider_res.json
    curl -sk "${PLATFORM_API_URL}${lf_apicpath}" \
      -H 'accept: application/json' \
      -H "authorization: Bearer $lf_am_token" \
      -H 'content-type: application/json' \
...
for index in ${!api_names[@]}
  do
    adapt_file ${MY_APIC_SIMPLE_DEMODIR}config/ ${MY_APIC_GEN_CUSTOMDIR}config/ ${api_files[$index]}
...
function apic_run_all () {
  ...
  adapt_file ${MY_APIC_SIMPLE_DEMODIR}properties/ ${MY_APIC_GEN_CUSTOMDIR}properties/ apic.properties
  adapt_file ${MY_APIC_SIMPLE_DEMODIR}config/ ${MY_APIC_GEN_CUSTOMDIR}config/ web-mgmt.cfg
  
  read_config_file "${MY_APIC_GEN_CUSTOMDIR}properties/apic.properties"
