...
function create_qmgr_certificate () {
  ...
  create_oc_resource "Certificate" "${VAR_QMGR}-cert" "${sc_mq_kafka_demo_yaml_dir}" "${sc_mq_kafka_demo_workingdir}" "qmgr_CACertificate.yaml" "${sc_mq_kafka_demo_workingdir}"
...
function create_oc_objects() {
  ...
  for ((i = 0; i < lf_length; i += 2)); do
    ...
    create_oc_resource "ConfigMap" "$lf_cm_id" "${lf_in_source_directory}" "${lf_in_target_directory}" "$lf_file" "${lf_in_namespace}"
  done
  
  trace_out 3 create_oc_objects
}

#############################################################
function create_qmgr_configmaps_old () {
  trace_in 3 create_qmgr_configmaps_old
  ...
  trace_out 3 create_qmgr_configmaps_old
}
...
function create_clnt_kdb () {
  ...
  case $VAR_KEYDB_TYPE in
    cms)  local lf_clnt_keydb="${sc_mq_kafka_demo_workingdir}${CLNT1}-keystore.kdb";;
    pkcs12) local lf_clnt_keydb="${sc_mq_kafka_demo_workingdir}${CLNT1}-keystore.p12";;
  esac  
...
function mq_kafka_demo_generate_certs() {
  trace_in 3 mq_kafka_demo_generate_certs
...
function create_kafka_topic() {
  ...
  trace_out 3 create_kafka_topic
}

################################################
# Create Kafka user
function create_kafka_user() {
  trace_in 3 create_kafka_user
...
function mq_kafka_demo_run_all () {
  trace_in 3 mq_kafka_demo_run_all
  ...
  trace_out 3 mq_kafka_demo_run_all
}

################################################
# initialisation
function mq_kafka_demo_init() {
  trace_in 2 mq_kafka_demo_init
  ...
  export VAR_MQSC_LDAP_CM="${VAR_QMGR}-ldap-cm"
  export VAR_JMS_CM="${VAR_QMGR}-jms-cm"
  export VAR_MQSC_MODIFY="${VAR_QMGR}-modify-cm"
