...
function eem_run_all () {
  ...
  decho 4 "oc -n $VAR_APIC_NAMESPACE get APIConnectCluster -o=jsonpath='{.items[?(@.kind=="APIConnectCluster")].status.endpoints[?(@.name=="jwksUrl")].uri}'"
  ...
  if ! oc -n ${VAR_EEM_NAMESPACE} get secret $VAR_EEM_APIC_INGRESS_CA_CERT_SECRET_NAME >/dev/null 2>&1; then
    mylog info "Configure Event Endpoint Management to trust API Connect"
  
    decho 4 "oc -n $VAR_APIC_NAMESPACE get secret ${VAR_APIC_INSTANCE_NAME}-ingress-ca -o=jsonpath=\"{.data['ca\.crt']}\""
    ...
    create_oc_resource "Secret" "$VAR_EEM_APIC_INGRESS_CA_CERT_SECRET_NAME" "${MY_EEM_SCRIPTDIR}config/" "${MY_EEM_GEN_CUSTOMDIR}config/" "apim-cpd.yaml" "$VAR_EEM_NAMESPACE"
    ...
    decho 4 "oc -n $VAR_EEM_NAMESPACE patch EventEndpointManagement "${VAR_EEM_INSTANCE_NAME}" --type merge -p '{\"spec\": {"manager": {\"apic\": {\"jwks\": { \"endpoint\": \"$lf_jwks_url\"}}}}}'"
    oc -n $VAR_EEM_NAMESPACE patch EventEndpointManagement "${VAR_EEM_INSTANCE_NAME}" --type merge -p "{\"spec\" : {\"manager\": {\"apic\": {\"jwks\": {\"endpoint\": \"$lf_jwks_url\"}}}}}"
    # TODO  name of instance (apim-cpd) should in  a variable
    decho 4 "oc -n $VAR_EEM_NAMESPACE patch EventEndpointManagement "${VAR_EEM_INSTANCE_NAME}" --type merge -p \"{\"spec\":  {\"manager\": {\"tls\": {\"trustedCertificates\": [{\"certificate\": \"ca.crt\"},{\"secretName\": \"$VAR_EEM_APIC_INGRESS_CA_CERT_SECRET_NAME\"}]}}}}\""
    oc -n $VAR_EEM_NAMESPACE patch EventEndpointManagement "${VAR_EEM_INSTANCE_NAME}" --type merge -p '{"spec":{"manager":{"tls":{"trustedCertificates":[{"certificate":"ca.crt","secretName":"apim-cpd"}]}}}}'
  fi
...
export VAR_EEM_WORKINGDIR="${sc_component_script_dir}working/"
...
sc_component_properties_file="${sc_component_script_dir}../config/eem.properties"
