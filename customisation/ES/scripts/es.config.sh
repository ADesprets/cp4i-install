...
function create_es() {
  trace_in 3 create_es

  # ibm-eventstreams
  if $MY_ES; then

    # Creating Event Streams instance
    create_operand_instance "EventStreams" "${VAR_ES_INSTANCE_NAME}" "${MY_OPERANDSDIR}" "${VAR_ES_WORKINGDIR}" "ES-Capability.yaml" "$VAR_ES_NAMESPACE" "{.status.phase}" "Ready"
  ...
  trace_out 3 create_es
}

#####################################
# Create kafka topics
function create_kafka_topics () {
  trace_in 3 create_kafka_topics

  local lf_source_directory="${MY_ES_SCRIPTDIR}config/"
  ...
  trace_out 3 create_kafka_topics
}

#####################################
# Create kafka users
function create_kafka_users () {
  trace_in 3 create_kafka_users
  ...
  trace_out 3 create_kafka_users
}

#####################################
# Create kafka connector 
function create_kafka_connector () {
  trace_in 3 create_kafka_connector
  ...
  trace_out 3 create_kafka_connector
}

#####################################
# run all
function es_run_all () {
  trace_in 3 es_run_all
  ...
  # Create namespace 
  create_project "$VAR_ES_NAMESPACE" "$VAR_ES_NAMESPACE project" "For MQ customisation" "${MY_RESOURCESDIR}" "${VAR_ES_WORKINGDIR}"
...
export VAR_ES_WORKINGDIR="${sc_component_script_dir}working/"
